...
Format JSON yang diwajibkan:
{
  ...
  "total_budget": 5000000,
  "weather_prediction": "Cerah, suhu 25-30C. Saran: Bawa topi dan kacamata.",
  "packing_list": ["Kacamata hitam", "Sunblock", "Sepatu nyaman"],
  "days": [
    {
      "dayNumber": 1,
      "items": [
...
Aplikasi ini memunculkan lokasi pada peta menggunakan OpenStreetMap. Oleh karena itu, nilai "locationName" HARUS merupakan nama tempat/landmark asli yang spesifik, resmi, dan terdaftar di OpenStreetMap agar bisa dicari oleh sistem (contoh: "Candi Prambanan", "Pantai Kuta, Bali", bukan sekadar "Makan siang di restoran" atau "Jalan-jalan di pusat kota"). Gunakan nama asli tempat wisata atau restoran tersebut.${personalizationInstructions}`;
    ...
    try {
      ...
      let finalNotes = tripData.notes || "Auto-generated by AI";
      
      // Convert Expenses to Notes (Estimations) instead of actual spent expenses
      if (tripData.expenses && Array.isArray(tripData.expenses) && tripData.expenses.length > 0) {
        finalNotes += "\n\n=== Estimasi AI ===\n";
        for (const exp of tripData.expenses) {
          const expAmount = parseFloat(String(exp.amount).replace(/[^\d.-]/g, ''));
          if (!isNaN(expAmount)) {
            finalNotes += `${exp.title || "Pengeluaran"}::${expAmount}\n`;
          }
        }
      }

      // 1. Insert Trip
      const weatherPred = tripData.weather_prediction || null;
      const packList = tripData.packing_list && Array.isArray(tripData.packing_list) ? JSON.stringify(tripData.packing_list) : null;
      
      const [tripResult] = await connection.execute(
        `INSERT INTO trips (user_id, name, destination, start_date, end_date, notes, weather_prediction, packing_list) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [userId, tripName, tripDest, startDate, endDate, finalNotes, weatherPred, packList]
      );
      ...
      // 3. (Dihapus: Pengeluaran AI tidak lagi dicatat sebagai pengeluaran riil)
